...
function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('http://-backend.onrender.com/api/reports', {
        method: 'POST',
        body: formData
      });
...
function ReportsList() {
  ...
  useEffect(() => {
    fetch('http://voice-for-the-voiceless-backend.onrender.com/api/reports?onlyOpen=true')
      .then((res) => {
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      const res = await fetch(`http://voice-for-the-voiceless-backend.onrender.com/api/reports/${id}/status`, {
        method: 'PATCH',
      ...
      <tbody>
        {reports.map((r) => (
          <tr key={r.id}>
            ...
            <td>
              {r.image_path ? (
                <a href={`http://voice-for-the-voiceless-backend.onrender.com{r.image_path}`} target="_blank" rel="noreferrer">
                  View photo
                </a>
              ) : (
                'No photo'
